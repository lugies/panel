...
<body style="background: url('../img/bg-3.jpg')">
    ...
    <div class="card bg-info">
        <div class="card-body text-center">
            ...
            <?php
                $users_id = $_GET['id'];
                $query = $db->query("SELECT * FROM users WHERE users_id = $users_id");
                $query->execute();
                $all = $query -> fetchAll();
                    foreach( $all as $row ){
            ?>
            <?php
                    ...
                    <div class="col-lg-12">';
                    echo "<input class='form-control'  type='text' value='".$row['users_id']."' disabled>";
                    echo '</div>
                    </div>';
            ?>
            <?php
                    echo '<div class="form-group">
                    <div class="col-lg-12">';
                    echo "<input class='form-control'  type='text' value='".$row['users_name']."'>";
                    echo '</div>
                    </div>';
            ?>
            <?php
                    echo '<div class="form-group">
                    <div class="col-lg-12">';
                    echo "<input class='form-control'  type='number' value='".$row['users_phone']."'>";
                    echo '</div>
                    </div>';
            ?>
            <?php
                    echo '<div class="form-group">
                    <div class="col-lg-12">';
                    echo "<input class='form-control'  type='text' value='".$row['users_date']."'>";
                    echo '</div>
                    </div>';
                    }
            ?>               
